...
type ClientMessage =
  | { type: 'join'; avatar: string; size?: PlayerCount }
...
type ServerMessage =
  ...
  | {
      ...
      size: PlayerCount
      host: number | null
      seats: ({ avatar: string; connected: boolean; confirmed: boolean } | null)[]
    }
  | { type: 'move'; move: Move }
...
const createRoom = (size: PlayerCount, hostSid: string): RoomData => ({
  size,
  hostSid,
...
const lobbyMessage = (store: RoomData): ServerMessage => {
  ...
  return {
    ...
    seats: store.seats.map((seat) =>
      seat === null ? null : { avatar: seat.avatar, connected: seat.connected, confirmed: seat.confirmed },
    ),
  }
}
...
export default class ScopaServer implements Party.Server {
  ...
  async onMessage(message: string, sender: Party.Connection): Promise<void> {
    ...
    if (data.type === 'join') {
      const store =
        existing ??
        createRoom(typeof data.size === 'number' && isPlayerCount(data.size) ? data.size : DEFAULT_SIZE, sid)
      ...
      const seats = seatPlayer(store.seats, {
        sid,
        avatar: data.avatar,
        connected: true,
    ...
    switch (data.type) {
      ...
      case 'start': {
        ...
        return this.broadcast({ type: 'state', state: game })
      }
      ...
      case 'move': {
        ...
        return this.room.broadcast(JSON.stringify({ type: 'move', move: data.move } satisfies ServerMessage), [
          sender.id,
        ])
      }
      ...
      case 'confirm': {
        ...
        await this.save({
          ...store,
          seats: seats.map((seat) => (seat === null ? null : { ...seat, confirmed: false })),
          game,
        })
        return this.broadcast({ type: 'state', state: game })
      }
  ...
  async onClose(connection: Party.Connection): Promise<void> {
    ...
    if (store.game?.state === 'stop' && allConfirmed(seats)) {
      const game = dealRound(store.size, store.game.firstPlayer, store.game.score)
      await this.save({
        ...next,
        seats: seats.map((seat) => (seat === null ? null : { ...seat, confirmed: false })),
